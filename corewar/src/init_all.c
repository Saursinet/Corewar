...
** Last update Thu Apr 16 20:27:25 2015 Florian Saurs
*/
...
void		verif_number(t_game game)
{
  ...
  a = game.champ_alive[0][0];
  b = game.champ_alive[1][0];
  c = (i >= 2 ? game.champ_alive[2][0] : -3);
  d = (i >= 3 ? game.champ_alive[3][0] : -4);
  (a == b || a == c || a == d) ? my_puterror_nbr(a) : (b == c || b == d) ?
    my_puterror_nbr(b) : c == d ? my_puterror_nbr(c) : 0;
}
